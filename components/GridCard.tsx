...
export function GridCard({ href, title, actionLabel = "Open" }: { href: string; title: string; actionLabel?: string }) {
  return (
    <Link
      href={href}
      className="group flex h-full w-full aspect-square flex-col justify-between rounded-2xl border bg-card p-4 sm:p-5 shadow-sm transition-all hover:bg-accent/60 hover:border-border/80 hover:shadow-md active:scale-[0.98]"
    >
